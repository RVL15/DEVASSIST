...
function getFileContext(editor: vscode.TextEditor) {
...
class DevAssistCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  async provideInlineCompletionItems(
    ...
    if (!vscode.workspace.getConfiguration("devassist").get("autocompleteEnabled", true)) return;
    ...
    return new Promise((resolve) => {
      ...
      this.debounceTimer = setTimeout(async () => {
        ...
        try {
          const data = await post<{ completion: string }>("autocomplete", {
            prefix, suffix, file_context: fileContext, max_tokens: 128,
          });
          if (!data.completion || token.isCancellationRequested) { resolve(undefined); return; }
...
function createResultPanel(context: vscode.ExtensionContext, title: string, content: string, lang = "plaintext") {
...
function openChatPanel(context: vscode.ExtensionContext) {
  const panel = vscode.window.createWebviewPanel("devassistChat", "DevAssist Chat",
  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    if (msg.type === "send") {
      ...
      try {
        const data = await post<{ reply: string; latency_ms: number }>("chat", {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.languages.registerInlineCompletionItemProvider(
      { pattern: "**" },
      new DevAssistCompletionProvider()
    )
  );
  ...
  context.subscriptions.push(vscode.commands.registerCommand("devassist.explain", async () => {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "DevAssist: Explaining..." }, async () => {
      try {
        const data = await post<{ explanation: string }>("explain", { code, language: lang });
  ...
  context.subscriptions.push(vscode.commands.registerCommand("devassist.refactor", async () => {
    ...
    const goals = await vscode.window.showQuickPick(
      ["readability", "performance", "security", "dry"],
      { canPickMany: true, placeHolder: "Select refactor goals" }
    );
    if (!goals?.length) return;
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "DevAssist: Refactoring..." }, async () => {
      try {
        const data = await post<{ refactored_code: string; changes: string[] }>("refactor", {
  ...
  context.subscriptions.push(vscode.commands.registerCommand("devassist.generate", async () => {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "DevAssist: Generating..." }, async () => {
      try {
        const data = await post<{ code: string; explanation: string }>("generate", {
